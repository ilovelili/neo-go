...
const (
	// ModeMainNet the Magic field in ProtocolConfiguration. Check your protocol.json
	ModeMainNet NetMode = 0x00746e41 // 7630401
	// ModeTestNet check your protocol.test.json
	ModeTestNet NetMode = 0x74746e41 // 1953787457
	// ModePrivNet docker private net
...
var (
	...
	// BuildTime the time and date the current version of the node built, set at build time.
	BuildTime string
)
...
// Load load the config from the give path and netMode.
func Load(path string, netMode NetMode) (Config, error) {
	configPath := fmt.Sprintf("%s/protocol.%s.yaml", path, netMode)
